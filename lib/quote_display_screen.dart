  ...
  void _rate(String quoteId, int rating, BuildContext context) async {
    try {
      await _firestore
          ...
          .collection('ratings')
          .doc(FirebaseAuth.instance.currentUser!.uid)
          .set({'rating': rating, 'timestamp': FieldValue.serverTimestamp()});
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Quote rated: $rating')));
    } catch (e) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Error rating: ${e.toString()}')));
    }
  }
